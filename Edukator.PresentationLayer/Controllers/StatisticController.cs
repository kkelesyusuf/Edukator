...
namespace Edukator.PresentationLayer.Controllers
{
    public class StatisticController : Controller
    {
        public IActionResult Index()
        {
            ...
            ViewBag.last3TotalPrice = context.Courses.OrderByDescending(x => x.CategoryID).Take(3).Average(y => y.Price);
            ViewBag.getCourseCategorySoftwareAndReviewMore9Score = context.Courses.Where(x => x.CategoryID == (context.Categories.Where(y => y.CategoryName == "Yazılım").Select(z => z.CategoryID).FirstOrDefault()) && x.Review >= 4).Select(w => w.Title).FirstOrDefault();
